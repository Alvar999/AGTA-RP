...
timer NOPCheck[5000](playerid)
{
    ...
    else if(IsAPizzaCar(newcar) && PlayerInfo[playerid][pJob] != 3 && PlayerInfo[playerid][pJob2] != 3) ExecuteNOPAction(playerid);
    else if(IsVIPcar(newcar) && PlayerInfo[playerid][pDonateRank] == 0) ExecuteNOPAction(playerid);
    else if(IsATruckerCar(newcar) && PlayerInfo[playerid][pJob] != 2 && PlayerInfo[playerid][pJob2] != 2) ExecuteNOPAction(playerid);
...
task ResetHaiCan[900000]()
{
	...
	foreach(new i: Player)
	{
		if (PlayerInfo[i][pAdmin] < 2 || HelpingNewbie[i] != INVALID_PLAYER_ID)
		{
			...
			{
				if(PlayerInfo[i][pThirsty] > 0) // 30 minutes
				{
					PlayerInfo[i][pThirsty] -= 5;
					if (PlayerInfo[i][pThirsty] < 0)
						PlayerInfo[i][pThirsty] = 0;

					if (PlayerInfo[i][pThirsty] == 0)
					{
						SendClientMessageEx(i, COLOR_LIGHTBLUE, "Ban dang rat khat nuoc. Ban phai bo sung nuoc ngay bay gio.");
					}
				}

				if(PlayerCuffed[i] == 0)
				{
					if (PlayerInfo[i][pThirsty] == 0) // 10 minutes
					{
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se bi gioi han 2 gio choi dau cho nguoi moi tham gia cong dong AGTA-RP.");
		}
		if(PlayerInfo[i][pLevel] >= 1) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Thong Bao: Ban Chua Co Giay Phep Su Dung xe , thuyen , may bay , Hay Nhap /MAP => Tim DUong => Thi Bang Lai De Co Bang Lai nhe !");
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Neu Ban Da Co Bang Lai Hay Bo Qua Tin Nhan Nay Nhe !");
		}
		if(PlayerInfo[i][pGiayPhepVuKhi] < 1) {
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "{23FF55}Ban chua co giay phep su dung vu khi hay lam ho so");
		}
	...
	foreach(new i: Player)
	{
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0) {
				...
				PhoneOnline[i] = 0;
				SendClientMessageEx(i, COLOR_GRAD1,"[SvG] Ban da tra no cho xa hoi.");
				GameTextForPlayer(i, "~g~Tu do~n~~w~Hay Co Gang Lam Mot Nguoi Tot", 5000, 1);
				ClearCrimes(i);
		...
		if (PlayerInfo[i][pAdmin] < 2 || HelpingNewbie[i] != INVALID_PLAYER_ID)
		{
			if (PlayerInfo[i][pHospital] == 0 && GetPVarInt(i, "Injured") != 1 && GetPVarInt(i, "IsFrozen") == 0 && GetPVarInt(i, "PlayerCuffed") == 0)
			{
				if (++PlayerInfo[i][pHungerTimer] >= 1800 && PlayerInfo[i][pHunger] > 0) // 30 minutes
				{
					...
					PlayerInfo[i][pHunger] -= 20; // cang tang thi thanh doi bung cang het nhanh
...
task ServerMicrobeat[500]() {
	...
	new str_time[128], mtext[20], thour, year, month,day;
	...
	if(hour > 12 && hour < 24) thour = hour - 12;
	else if(hour == 12) thour = 12;
	else if(hour > 0 && hour < 12) thour = hour;
	else if(hour == 0) thour = 12;

	format(str_time, sizeof(str_time), "%d/%s/%d - %d:%02d~n~|AGTA-RP.NET|", day, mtext, year, thour, minuite);
	TextDrawSetString(ServerTime, str_time);

	foreach(new i: Player)
	{
		if(gPlayerLogged{i} == 1) TextDrawShowForPlayer(i, ServerTime);
		...
		switch(GetPlayerState(i)) {
		    case PLAYER_STATE_DRIVER: {
				...
				else if(PlayerInfo[i][pSpeedo] != 0 && FindTimePoints[i] == 0 && arr_Engine{iVehicle} == 1 && !GetPVarType(i, "LoadTruckTime") && _vhudVisible[i] == 1) {
			...
			case PLAYER_STATE_PASSENGER: {
				...
				fCurrentSpeed = player_get_speed(i);
				if(PlayerInfo[i][pSpeedo] != 0 && _vhudVisible[i] == 1) {
					UpdateVehicleHUDForPlayer(i, floatround(VehicleFuel[iVehicle]), floatround(fCurrentSpeed));
				}
				if(fCurrentSpeed >= 40 && 60 <= fCurrentSpeed)
				{
	...
	if(TimeTaiXiu == 0)
	{
	    ...
	    foreach(new i: Player)
		{
			if(TimeChonTaiXiu[i] < 10)
			{
			    ...
			    {
					format(string,sizeof(string),"{86fb12}[tai xiu]ban duoc tra lai %d$ va loai khoi phien nay de can bang giua tai va xiu",TienCuocTaiXiu[i]);
            		SendClientMessageEx(i, -1, string);
            		SendClientMessageEx(i, -1, "{86fb12}[TAI XIU] Co gang tham gia som hon de khong bi loai ra nhe");
            		GivePlayerCash(i,TienCuocTaiXiu[i]);
                    TienCuocTraAll = TienCuocTraAll + TienCuocTaiXiu[i];
				}
				if(TienCuocTaiAll == 0 && KetQuaTaiXiu == 1)
				{
            		format(string,sizeof(string),"{86fb12}[TAI XIU] Phien so %d tuyen bo: %s thang( Tong tien cuoc: %s$)",Phientaixiu,ketqua,number_format(totalwealth));
            		SendClientMessageEx(i, -1, string);
				}
				if(TienCuocTaiAll > 0 && KetQuaTaiXiu == 1)
				{
            		format(string,sizeof(string),"{86fb12}[TAI XIU] Phien so %d tuyen bo: %s thang( Tong tien cuoc: %s$ - An nhieu nhat: %s [%s$])",Phientaixiu,ketqua,number_format(totalwealth),GetPlayerNameEx(IdTaiWin),number_format(TienIdTaiWin*2));
            		SendClientMessageEx(i, -1, string);
				}
				...
				if(TienCuocXiuAll > 0 && KetQuaTaiXiu == 2)
				{
            		format(string,sizeof(string),"{86fb12}[TAI XIU] Phien so %d tuyen bo: %s thang( Tong tien cuoc: %s$ - An nhieu nhat: %s [%s$])",Phientaixiu,ketqua,number_format(totalwealth),GetPlayerNameEx(IdXiuWin),number_format(TienIdXiuWin*2));
            		SendClientMessageEx(i, -1, string);
				}
            	ChonTaiXiu[i] = 0;
				TimeChonTaiXiu[i] = 0;
			...
			if(TimeChonTaiXiu[i] >= 10)
			{
				if(ChonTaiXiu[i] == KetQuaTaiXiu)
				{
	            	...
	            	format(string,sizeof(string),"{86fb12}[TAI XIU] ban da thang trong phien nay va ban nhan duoc %d$",TienCuocTaiXiu[i]*2);
	            	SendClientMessageEx(i, -1, string);
				...
				else if(ChonTaiXiu[i] != KetQuaTaiXiu)
				{
	            	...
	            	if(ChonTaiXiu[i] > 0)
	            	{
	            		SendClientMessageEx(i, -1, "{86fb12}[TAI XIU] ban da thua trong phien nay");
					}
	            	ChonTaiXiu[i] = 0;
					TimeChonTaiXiu[i] = 0;
					TienCuocTaiXiu[i ]= 0;
